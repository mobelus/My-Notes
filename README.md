...
### Как написать Указатель на переменную, которУЮ нельзя менять (конст. именно то куда указывает указатель):
Ответ: есть 2 идентичные записи:
```
int i2=2, i3=3;
int const* p2(&i2); // это идентично (->)
const int* p3(&i3); // (->) этому
```

### Указатель на переменную, которЫЙ нельзя менять (именно указатель коснтантный):
```
int i4 = 1;
int* const p4(&i4);
...
// ЕСЛИ  const  стоит рядом с ПЕРЕМЕННОЙ(переменная сразу справа от конст),
//  то константна именно переменная а не содержимое
// ЕСЛИ  const  cтоит перед ЗВЁЗДОЧКОЙ, значит 
//  константна "операция звёздочка", и НЕЛЬЗЯ обращаться по *p !!!
...
# Как поместить свой класс / своя структура в set /  Правильное примнение оператора сравненя (1)
...
ОТВЕТ: Нужно переопредлелить оператор < "меньше"?
...
# Неявные методы Класса по умолчанию:
```
class A
{
  public:
...
### Когда не обойтись без дружественных friend функций при перегрузке операторов?
...
struct Vector2
{
  ...
  // Дружественность необходима для того, чтобы 
  // мы имели доступ к приватным членам класса Vector3
  friend Vector2 operator + (Vector3 v3, Vector2 v2) {/*...*/ }
}
//Объекту Vector2 присваиваем сумму объектов Vector2 и Vector3
vec2 = vec2 + vec3; //Ok
vec2 = vec3 + vec2; //Ошибка без friend-operator-a
```

# Перегрузка всех 49-и операторов в C++

https://www.pvsm.ru/programmirovanie/181616
...
https://ru.wikipedia.org/wiki/C%2B%2B11#Лямбда-функции_и_выражения
...
# 7 — лямбды / лямбда функции / лямбда выражения
