...
# Как сгенерировать pure virtual call exeption / pure virtual call исключение ? 
...
Class operator + (Class &v1, Class &v2)
...
int Class::operator [] (int n)
void* Class::operator new(size_t v) //Выделяем память под 1 объект
void* Class::operator new[](size_t v) //Выделение памяти под несколько объектов
void Class::operator delete(void* v)
void Class::operator delete[](void* v)
```
...
# Многопоточность и std::thread / Threading stuff
...
# Createthread разница _beginthreadex / _beginthread
...
public:
  ...
  // not here // COPY !!!
  // not here // AND
  //std::swap(m_data, that.m_data); // SWAP !!!	
  // Если с  (CONST MyString that), то swap ругнётся can not convert!!!  char*, char* const
    return *this;
  }
...
Бывают нескольких видов, например - Простые владеющие указатели, которые при присвоении нового значения или удалении сами удаляют объект. Еще пример - Указатели с подсчётом ссылок
...
Можно вручную указать возвращаемый тип лямбда-функии, используя синтаксис "-> тип":
...
Парметры **ЗАМКНУТЫ** или находятся **внутри ЗАМЫКАНИЯ** ЗАМЫКАНИЕ -
доступ извне к ним более невозможен, они доступны только в рамках теля лямбда выражения.

-> - тип возвращаемого значения
result_type
это тип возвращаемого значения. Это может показаться несколько непривычно, так как раньше тип всегда писали перед сущностью(переменной, функцией).
...
### Как получить значение которое возвращает лямбда цункция?
...
1. Виртуальный конструктор - это просто название для ПАТТЕРНА ПРОЕКТИРОВАНИЯ под названием **ФАБРИЧНЫЙ МЕТОД** 
2. В обычном классе конструктор вритуальным быть НЕ может. Это бред с точки зрения практики и концепции неаследования.
...
**new-expression** — это оператор языка, такой же как if, while и т.д. (хотя if, while и т.д. все же именуются как statement, но отбросим лирику) Т.е. встречая его в листинге компилятор генерирует определенный код, соответствующий этому оператору. Так же new — это одно из key-words языка С++, что еще раз подтверждает его общность с if'ами, for'ами и т.п. 

А **operator new()** в свою очередь — это просто одноименная функция языка С++, поведение которой можно переопределить. 

**ВАЖНО** — **operator new()** НЕ вызывает конструктор(ы) для объекта(ов), под который(ые) выделяется память. Он просто выделяет память нужного размера и все. Его отличие от сишных функций в том, что он может бросить исключение **std::bad_alloc** и его можно переопределить, а так же сделать оператором для отдельно взятого класса, тем самым переопределить его только для этого класса (остальное вспомните сами:)).

А вот **new-expression** как раз и вызывает конструктор(ы) объекта(ов). Он фактически является оператоором препроцессора, ибо встречая его, компилятор генерирует код вызова конструктора(ов) - Хотя правильней сказать, что он тоже ничего не вызывает, просто, встречая его, компилятор генерирует код вызова конструктора(ов).


# MALLOCK
``` void * malloc(size_t size); ```
malloc() – метод, Возвращает указатель на выделенную память, или NULL, если память выделить не удалось. Так как malloc возвращает указатель типа void, то его необходимо явно приводить к нужному нам типу. Обычно malloc() для выделения памяти используют совместно с функцией sizeof().
...
# Какие типы new / типы оператора new / виды оператора new / разновидности оператора new вам известны:
...
- new() является препроцессором, в то время как malloc() – методом.
- Пользователю нет необходимости знать размер под выделяемый объект при использовании 'new',
а в случае с malloc() для выделения памяти порой стоит использовать функцию sizeof().
...
- «new» инициализирует новую память в 0, в то время как malloc() сохраняет случайное значение в новой выделенной ячейке памяти.
- Если new не может выделить память в куче, то он бросит (throw) исключение типа std::bad_alloc, А malloc вернёт NULL, если память выделить не удалось.
- Соответ-нно delete (для new) вызывает деструктор и освобождает память. Для malloc() нужно вызывать ф-цию free(), которая просто освобождает память. 
...
Различий ДВА:
1. У Структуры все поля по умолчанию public, у Класса все поля по умолчанию private
2. У Структуры тип наследования по умолчанию public, у Класса тип наследования по умолчанию private

На вопрос, "Какие ключевые слова означают класс ?" верный ответ: class и struct
...
### Зачем нужна директива #pragma pack / #pragma pack(1)
- указание того, что можно не использовать выравнивание
- указать размер выравнивания, которое мы желаем, чтобы оно действовало именно такое, для какого-то участка кода
- упаковка структур / выравнивание
```
Пример:
struct Foo { int i; char a; };
int size = sizeof(Foo); //sizeof вернёт 8 байт

Как мы думаем:    |i|i|i|i|a|
Как в реальности: |i|i|i|i|_|_|_|a|
```
sizeof вернёт 8 байт, хотя int (4) + char (1) = 5. 
- Все дело в дефолтном выравнивании равном 4, т.е. размер ячеек памяти должен быть кратен блоку в 4 байта.
  
### 56. Что такое выравнивание и от чего оно зависит? Влияет ли выравнивание на размер класса? #pragma pack

https://tproger.ru/translations/art-of-structure-packing/

**Коротко:** 
Расположение данных в памяти поблочно, так как процессор читает машинными словами. Зависит от реализации компилятора, ОЗУ и процессора. Влияет. Процессору можно сказать не использовать выравнивание, с помощью директивы **#pragma pack**

**Подробно:**
На процессорах x86 и ARM примитивные типы не могут находиться в произвольной ячейке памяти. Каждый тип, кроме char, требует выравнивания. char может начинаться с любого адреса, однако двухбайтовый short должен начинаться только с четного адреса, четырехбайтный int или float — с адреса, кратного 4, восьмибайтные long или double — с адреса, кратного 8. Наличие или отсутствие знака значения не имеет. Указатели — 32-битные (4 байта) или 64-битные (8 байт) — также выравниваются. Выравнивание ускоряет доступ к памяти за счет генерации кода, в котором на чтение и запись ячейки памяти требуется по одной инструкции.

**Без выравнивания** мы можем столкнуться с ситуацией, когда процессору придется использовать две и более инструкции для доступа к данным, расположенным между адресами, кратными размеру машинного слова. 

Единственная хорошая причина использования **#pragma pack** — если вы хотите, чтобы расположение данных в памяти точно соответствовало требованиям низкоуровневого протокола или оборудования

```
#pragma pack (push, 1)
struct { char f1; int f2; short f3; };
#pragma pack (pop)
```
...
C++ позволяет указывать несколько определений функций или операторов в одной области видимости для нормальной работы в пользовательских классах. Это называется перегрузкой функций и операторов.
...
Для фактически определения одного класса внутри другого. firend классу становятся доступны все (public , protected  и private) члены класса друга.
...
ОТВЕТ:  
- Копирование
- конструирование внутренних объектов
...
ОТВЕТ:  статические интегральные прямо на месте, остальные в инициализаторе или вне класса, или в конструкторе
...
--------------------------------------------
# ВИРТУАЛЬНАЯ ФУНКЦИЯ / ВИРТУАЛЬНЫЕ ФУНКЦИИ
...
```
Случай, когда в присваивании участвует два объекта из разных уровней одной иерархии.
Одному из объектов присваивается только базовая часть (то, что он знает) от другого объекта.
